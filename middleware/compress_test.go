...
import (
	...
	"compress/gzip"
	"fmt"
	"github.com/SirMetathyst/go-chi/v5"
	"io"
	"io/ioutil"
	"net/http"
	"net/http/httptest"
...
func TestCompressor(t *testing.T) {
	...
	compressor := NewCompressor(5, "text/template", "text/css")
	...
	r.Get("/gethtml", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/template")
...
func TestCompressorWildcards(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "no wildcard",
			types:      []string{"text/plain", "text/template"},
			typesCount: 2,
		},
		{
